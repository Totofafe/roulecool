...
def calculate_comfort_score(vibration):
    ...
    Seuils calibrés pour répartition ~1/3 chacun sur voirie urbaine :
    - < 3.0  : Confortable
    - 3.0-6.0 : Acceptable
    - > 6.0   : Inconfortable
    """
...
def get_color_from_score(score):
    """Couleur selon niveau de confort"""
    colors = {
...
def create_map(df, output_file='trajet_confort.html'):
    ...
    date_trajet = datetime.fromtimestamp(df.iloc[0]['time']).strftime('%d/%m/%Y %H:%M')
    
    folium.Marker(
        [df.iloc[0]['latitude'], df.iloc[0]['longitude']],
        popup=folium.Popup(f'<b>🚴 Départ</b><br><span style="font-size:11px;color:#666">{date_trajet}</span>', max_width=180),
        icon=folium.Icon(color='green', icon='play', prefix='fa')
    ).add_to(m)
    
    date_arrivee = datetime.fromtimestamp(df.iloc[-1]['time']).strftime('%d/%m/%Y %H:%M')
    
    folium.Marker(
        [df.iloc[-1]['latitude'], df.iloc[-1]['longitude']],
        popup=folium.Popup(f'<b>🏁 Arrivée</b><br><span style="font-size:11px;color:#666">{date_arrivee}</span>', max_width=180),
